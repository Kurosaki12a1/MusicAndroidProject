...
public class SongGenreAdapter extends RecyclerView.Adapter<SongGenreAdapter.ViewHolder> {
    ...
    public SongGenreAdapter(Context context, List<SongPlayerOnlineInfo>listSong){
    ...
    @Override
    public SongGenreAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_song_online_list, parent, false);

        SongGenreAdapter.ViewHolder viewHolder = new SongGenreAdapter.ViewHolder(view);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(SongGenreAdapter.ViewHolder holder, int position) {
